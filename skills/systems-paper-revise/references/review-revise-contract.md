...
Use Grill when a needed repair depends on unresolved author intent, competing technical interpretations, an absent scientific premise, actual experiment outcomes, a shared prior-work root cause, placement against unseen context, or a decision to change the claim or paragraph structure. Review marks these findings `author decision required` or evidence-blocked and gives the exact unit IDs. Ordinary grammar, category alignment, redundancy, and unambiguous local logic repairs need no interview when they preserve meaning. In particular, if prose says a property cannot replace proof of already named properties, Revise may align the comparison to those object-level properties while preserving the original claim strength and leaving its evidence blocked. Review can report the rest of its findings; Revise completes every safe local subrepair while leaving the blocked proposition or structural operation unapplied.

When discussion is requested or the author agrees to clarify a blocker, use [systems-paper-grill](../../systems-paper-grill/SKILL.md). It asks paper-specific questions and records the answers using the [decision-record rules](../../systems-paper-grill/references/decision-record.md). A read-only Review request does not authorize record writes or start revision. Keep the discussion within the supplied manuscript scope and shared standard.
